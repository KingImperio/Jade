...
description: "Authoritative reference for Jade terminal commands and command families"
---
...
| `--profile <name>`, `-p <name>` | Select which Jade profile to use for this invocation. Overrides the sticky default set by `hermes profile use`. |
...
| `hermes backup` | Back up Jade home directory to a zip file. |
...
| `hermes import` | Restore a Jade backup from a zip file. |
...
| `hermes acp` | Run Jade as an ACP server for editor integration. |
| `hermes mcp` | Manage MCP server configurations and run Jade as an MCP server. |
| `hermes plugins` | Manage Jade plugins (install, enable, disable, remove). |
...
| `hermes profile` | Manage profiles — multiple isolated Jade instances. |
...
| `hermes uninstall` | Remove Jade from the system. |
...
Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active Jade chat session.
...
**`hermes model`** (run from your terminal, outside any Jade session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active Jade chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your Jade session first (`Ctrl+C` or `/quit`), then run `hermes model` from your terminal prompt.
:::
...
| `--name NAME` | `Jade` | Bot display name in Slack. |
...
hermes auth spotify                                      # Authenticate Jade with Spotify via PKCE
```
...
Outputs a compact, plain-text summary of your entire Jade setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
| Section | Details |
|---------|---------|
| **Header** | Jade version, release date, git commit hash |
...
The report includes system info (OS, Python version, Jade version), recent agent and gateway logs (512 KB limit per file), and redacted API key status. Keys are always redacted — no secrets are uploaded.
...
Create a zip archive of your Jade configuration, skills, sessions, and data. The backup excludes the hermes-agent codebase itself.
...
The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when Jade is running (WAL-mode safe).
...
Restore a previously created Jade backup into your Jade home directory. All files in the archive overwrite existing files in your Jade home; `--force` only skips the confirmation prompt that fires when the target already has a Jade installation.
...
View, tail, and filter Jade log files. All logs are stored in `~/.hermes/logs/` (or `<profile>/logs/` for non-default profiles).
...
Jade uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `hermes logs list` subcommand shows all log files including rotated ones.
...
- `--source well-known` lets you point Jade at a site exposing `/.well-known/skills/index.json`.
...
Starts Jade as an ACP (Agent Client Protocol) stdio server for editor integration.
...
Manage MCP (Model Context Protocol) server configurations and run Jade as an MCP server.

| Subcommand | Description |
|------------|-------------|
| `serve [-v\|--verbose]` | Run Jade as an MCP server — expose conversations to other agents. |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Jade](../guides/use-mcp-with-hermes.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server).
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Jade Plugin](../guides/build-a-hermes-plugin.md).
...
right now without waiting for the next Jade update.
...
Migrate your OpenClaw setup to Jade. Reads from `~/.openclaw` (or a custom path) and writes to `~/.hermes`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing Jade files on conflicts (default: refuse to apply when the plan has conflicts). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into Jade equivalents or **archived** for manual review.
...
Manage profiles — multiple isolated Jade instances, each with its own config, sessions, skills, and home directory.
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of Jade commands, subcommands, and profile names.
...
- **Legacy `hermes.service` warning.** If Jade detects a pre-rename `hermes.service` systemd unit (instead of the current `hermes-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| `hermes uninstall [--full] [--yes]` | Remove Jade, optionally deleting all config/data. |
