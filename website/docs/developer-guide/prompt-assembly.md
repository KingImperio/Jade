...
description: "How Jade builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Jade deliberately separates:
...
You are Jade, an AI assistant created by Nous Research.
...
```
You are Jade, an intelligent AI assistant created by Nous Research.
...
|----------|-------|-------------|-------|
| 1 | `.hermes.md`, `HERMES.md` | CWD up to git root | Jade-native project config |
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Jade already loads, rather than editing Python templates in place.
...
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Jade.
...
- if you want to change how Jade assembles prompts for everyone, change Python and treat it as a code contribution
