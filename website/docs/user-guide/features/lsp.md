...
Jade runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Jade
...
1. Jade captures a baseline of current diagnostics for the file.
...
…). Jade auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
`node_modules` tree — Jade installs both packages together when you
...
When `install_strategy: auto`, Jade installs binaries into
...
shared location — the staging dir is fully Jade-owned and is
...
Servers are kept alive for the life of the Jade process. There's
