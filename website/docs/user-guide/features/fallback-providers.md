...
Jade has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — Jade can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`fallback_model` (singular) is the legacy single-fallback key — Jade still honors it for back-compat. `fallback_providers` (plural, list) supports multiple fallbacks tried in order; `hermes fallback` writes to this key. When both are set, Jade merges them with `fallback_providers` taking priority.
:::
...
Jade uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
When a task's provider is set to `"auto"` (the default), Jade tries providers in order until one works:
...
If the resolved provider fails at call time, Jade also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
For `auxiliary.session_search`, Jade also supports:
...
`base_url` takes precedence over `provider`. Jade uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.
...
If no provider is available for compression, Jade drops middle conversation turns without generating a summary rather than failing the session.
