...
description: "Customize Jade's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Jade's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who Jade is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Jade now seeds a default `SOUL.md` automatically in:

```text
~/.hermes/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run Jade with a custom home directory, it will use:
...
- Jade creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Jade loads `SOUL.md` only from `HERMES_HOME`
- Jade does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Jade falls back to a built-in default identity
...
If Jade loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the Jade instance itself.

That also makes it easier to teach users:
- "Edit `~/.hermes/SOUL.md` to change Jade' default personality."
...
- how Jade should handle uncertainty, disagreement, or ambiguity
...
## What Jade injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Jade falls back to a built-in default identity ("You are Jade, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Jade ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Jade, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Jade its persistent default personality unless the overlay meaningfully changes it.
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Jade speaks
- `display.skin` and `/skin` affect how Jade looks in the terminal
