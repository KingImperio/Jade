...
Jade can drive your Mac's desktop — clicking, typing, scrolling,
...
window-focus bug for UTM workflows). Jade refreshes the binary in two
places so you don't get stuck on a stale release:

- **`hermes update`** — when you update Jade itself, if `cua-driver` is
...
Jade applies multi-layer guardrails:
...
Screenshots are expensive. Jade applies four layers of optimisation:
