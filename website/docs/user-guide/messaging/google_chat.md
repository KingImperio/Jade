...
sidebar_position: 12
title: "Google Chat"
description: "Set up Jade as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Jade to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Jade
...
download the file. Save it somewhere only Jade can read (e.g.,
...
Also grant `Pub/Sub Viewer` on the same subscription — Jade calls
...
- **App name**: whatever you want users to see ("Jade" is reasonable).
...
`ADDED_TO_SPACE` event that Jade uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Jade
...
Install the dependencies the Google Chat adapter needs (no Jade extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Jade is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Jade detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Jade session.
...
3. Download the JSON. Move it onto the host that runs Jade.
4. On the host, register the client with Jade:
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Jade isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: Jade will only attach the SA bearer
