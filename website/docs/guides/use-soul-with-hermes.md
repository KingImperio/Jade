---
sidebar_position: 7
title: "Use SOUL.md with Jade"
description: "How to use SOUL.md to shape Jade's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Jade

`SOUL.md` is the **primary identity** for your Jade instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Jade to feel like the same assistant every time you talk to it — or if you want to replace the Jade persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Jade should be
- what Jade should avoid stylistically
- how Jade should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Jade is and how Jade speaks
...
Jade now uses only the global SOUL file for the current instance:

```text
~/.hermes/SOUL.md
```

If you run Jade with a custom home directory, it becomes:
...
Jade automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Jade does not overwrite it
- if the file exists but is empty, Jade adds nothing from it to the prompt

## How Jade uses it

When Jade starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Jade feels.
...
Jade already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Jade should sound.

# Avoid
What Jade should not do.

# Defaults
How Jade should behave when ambiguity appears.
```
...
Then restart Jade or start a new session.
...
### I edited SOUL.md but Jade still sounds the same
...
### Jade is ignoring parts of my SOUL.md
