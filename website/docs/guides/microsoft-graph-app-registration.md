...
4. Fill in:
   - **Name:** `Jade Teams Meeting Pipeline` (or any name you'll recognize).
...
```powershell
# Create a policy scoped to the Jade app
New-CsApplicationAccessPolicy `
  -Identity "Jade-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Jade meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Jade-Meeting-Pipeline-Policy" `
...
Jade ships a Graph auth smoke-test. From your Jade install:
