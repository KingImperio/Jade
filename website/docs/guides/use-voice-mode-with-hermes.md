---
sidebar_position: 8
title: "Use Voice Mode with Jade"
description: "A practical guide to setting up and using Jade voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Jade
...
- you want Jade sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Jade.
...
## Step 1: make sure normal Jade works first
...
Jade supports both local and cloud speech stacks.
...
If you choose NeuTTS in the setup wizard, Jade checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Jade transcribes and responds
...
- asking Jade to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Jade loop.

## Tuning CLI behavior

### Silence threshold

If Jade starts/stops too aggressively, tune:
...
Jade stays a normal chat bot, but can speak replies.
...
- you want Jade to function like a portable research or ops assistant
...
Jade joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Jade detects speech boundaries
- transcripts are posted in the associated text channel
- Jade responds in text and audio
