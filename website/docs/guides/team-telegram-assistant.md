...
This tutorial walks you through setting up a Telegram bot powered by Jade that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Jade installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
...
:::tip
A $5/month VPS is plenty for running the gateway. Jade itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Jade Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Jade. DM me for help with code, research, debugging, and more.
   ```
...
For a full guide, see [Use SOUL.md with Jade](/docs/guides/use-soul-with-hermes).
