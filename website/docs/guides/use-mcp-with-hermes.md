---
sidebar_position: 6
title: "Use MCP with Jade"
description: "A practical guide to connecting MCP servers to Jade, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Jade

This guide shows how to actually use MCP with Jade in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Jade tool
- you want Jade to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Jade to internal APIs, databases, or company systems without modifying Jade core

Do not use MCP when:
- a built-in Jade tool already solves the job well
...
- Jade remains the agent
- MCP servers contribute tools
- Jade discovers those tools at startup or reload time
...
If you installed Jade with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Jade banner/status should show MCP integration when configured
- ask Jade what tools it has available
...
## WSL2: bridge Jade in WSL to Windows Chrome

This is the practical setup when:

- Jade runs inside WSL2
...
In this setup, Jade does **not** connect to Chrome directly. Instead:

- Jade runs in WSL
- Jade starts a local stdio MCP server
...
```text
Jade (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Jade stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Jade core browser transport
...
Then start a fresh Jade session or run:
...
Once loaded, Jade can use the MCP-prefixed browser tools directly. For example:
...
If Jade runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Jade from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Jade from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Jade:
...
So Jade will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Jade to reason over a bounded workspace.
...
Now Jade can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Jade core.
...
Because Jade now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
